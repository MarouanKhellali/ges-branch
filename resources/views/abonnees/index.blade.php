...
@section('content')
    <div class="container mx-auto px-4">
        <h1 class="text-2xl font-bold mt-8 mb-4">All Abonnees</h1>
                    ...
                    @foreach ($abonnees as $abonnee)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div id="{{ $abonnee->id }}-dropdown"
                                    class="z-10 w-44 hidden bg-white rounded divide-y divide-gray-100 shadow">
                                    <ul class="py-1 text-sm text-gray-700"
                                        aria-labelledby="{{ $abonnee->id }}-dropdown-button">
                                        <li>
                                            <!-- Show button -->
                                            <a href="{{ route('abonnees.show', $abonnee->id) }}" data-bs-toggle="modal"
                                                data-bs-target="#showModal{{ $abonnee->id }}"
                                                class="block py-2 px-4 hover:bg-gray-100">Show</a>
                                        </li>
                                        <li>
                                            <!-- Edit button -->
                                            <a href="{{ route('abonnees.edit', $abonnee->id) }}" data-bs-toggle="modal"
                                                data-bs-target="#editModal{{ $abonnee->id }}"
                                                class="block py-2 px-4 hover:bg-gray-100">Edit</a>
                                        </li>
                                    </ul>
                                    <div class="py-1">
                                        <!-- Delete form -->
                                        <form action="{{ route('abonnees.destroy', $abonnee->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
